				   ...
				   int fcr, int prim, int nroots)
{
	...
	/* Allocate the arrays */
	rs->alpha_to = calloc(1, (rs->nn + 1) * sizeof(uint16_t));
	if (rs->alpha_to == NULL)
		goto err;

	rs->index_of = calloc(1, (rs->nn + 1) * sizeof(uint16_t));
	if (rs->index_of == NULL)
		goto err;

	rs->genpoly = calloc(1, (rs->nroots + 1) * sizeof(uint16_t));
	if(rs->genpoly == NULL)
		goto err;
